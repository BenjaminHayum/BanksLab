function [dataTable] = spectralPowerChannelPlot(measure,DataSet,varargin)
%% Documentation
%
% Plots the average spectral power for a particular band across every
% contact/channel and both conditions
%
...
% two sets of pairs which are 'Band' and 'Condition'
%
...
for iPatient = 1:length(patientList)
    ...
    for iCell = 1:length(contact2avg.patientID)
        if strcmp(currPatient, contact2avg.patientID{iCell}) == 1
            if strcmp(mainConditions{1}, contact2avg.mapCondition{iCell}) == 1
                ...
                Contact2ROI = contact2avg.Contact2ROI{iCell};
            end
    ...
    if firstBoolean == 1 && secondBoolean == 1
        ...
        for iContact = 1:length(currContactsCell)
            ...
            currROI = Contact2ROI(currContact);
        ...
        if optionSetBoolean == 0
            sgtitle([band ' Band Average Channel Spectral Power']);
        elseif optionSetBoolean == 1
            sgtitle([optionSet ' ' band ' Band Average Channel Spectral Power']);
        end
    end
end

%% Saving Figure
if ~exist(['Z:\Ephys\AnalysisOutput\SpectralPower\Channel Color\' DataSet], 'dir')
    mkdir(['Z:\Ephys\AnalysisOutput\SpectralPower\Channel Color\' DataSet])
    error(['New Dataset Directory Created: ' DataSet ' --> Check if accidental before rerunning'])
end
fileOutPath = ['Z:\Ephys\AnalysisOutput\SpectralPower\Channel Color\' DataSet '\' band '_' optionSet '_' upper(mainConditions{1}) 'vs' upper(mainConditions{2}) '.fig'];
